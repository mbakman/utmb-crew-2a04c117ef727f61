#!/usr/bin/env bash
#
# build-planb.sh — render the UTMB 2026 Plan B detail sheet
# (Champex → Orsières → Trient → Vallorcine, plus the B1/B2 split and the
# where-to-eat table) to a clickable one-page PDF and a single 300 dpi PNG.
#
# Sibling of build-nightplan.sh and deliberately identical in mechanism:
# same Chrome-does-not-exit workaround, same freshness guard, same checks.
#
#   ./transport/build-planb.sh            # render everything
#   ./transport/build-planb.sh pdf        # PDF only
#   ./transport/build-planb.sh png        # PNG only (needs the PDF to exist)
#   ./transport/build-planb.sh --open     # render everything, then open the PDF
#
# Inputs :  transport/planb-detail.html   (self-contained; no network, no assets)
#           content is a verbatim transcription of transport/planb-detail-data.json
# Outputs:  transport/dist/UTMB-2026-PlanB-Detail.pdf   1 page, A4 PORTRAIT
#           transport/dist/UTMB-2026-PlanB-Detail.png   that page, 300 dpi
#
...
SRC="$HERE/planb-detail.html"
DIST="$HERE/dist"
PDF="$DIST/UTMB-2026-PlanB-Detail.pdf"
PNG="$DIST/UTMB-2026-PlanB-Detail.png"
...
# Expected clickable-link count in the finished PDF: live tracking + the four
# map pills drawn from links{} (Orsières gare, Trient village, CP13 Vallorcine,
# Le Café Comptoir, Auberge du Mont-Blanc) — every one of them a real <a href>.
# Guards against a refactor that turns the Navigate pills into plain text.
MIN_URI=5
...
[ -x "$CHROME" ] || { echo "build-planb.sh: no Chrome binary found. Set CHROME_BIN." >&2; exit 1; }

[ -f "$SRC" ] || { echo "build-planb.sh: missing source $SRC" >&2; exit 1; }
mkdir -p "$DIST"
...
PROFILE="$(mktemp -d "${TMPDIR:-/tmp}/utmb-planb-chrome.XXXXXXXX")"
...
run_chrome() { # outfile arg...
  ...
  ref="$(mktemp "${TMPDIR:-/tmp}/utmb-planb-ref.XXXXXX")"
  "$CHROME" "$@" >/dev/null 2>&1 &
  pid=$!
...
render_png() {
  ...
  [ -f "$PDF" ] || { echo "  FAIL  $PDF does not exist — run 'build-planb.sh pdf' first." >&2; return 1; }
...
echo "UTMB 2026 crew Plan B detail — build"
echo "  chrome: $CHROME"
echo "  source: $SRC"
echo
...
case "$TARGET" in
  ...
  *)   echo "usage: build-planb.sh [pdf|png|all|--open]" >&2; exit 2 ;;
esac
